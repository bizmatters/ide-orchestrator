...
class OrchestrationService:
    ...
    async def get_or_create_draft(self, workflow_id: str, user_id: str) -> str:
        """
        Get existing draft or create new one for workflow.
        ...
        with psycopg.connect(self.database_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                # First validate user has access to workflow
                cur.execute(
                    "SELECT id, name FROM workflows WHERE id = %s AND created_by_user_id = %s",
                    (workflow_id, user_id)
                )
    ...
    async def _process_refinement_async(
        ...
        """
        Async processing of refinement with deepagents-runtime.
        ...
        try:
            ...
            # Call deepagents-runtime invoke endpoint
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                # Get final state
                state_response = await client.get(f"{self.deepagents_url}/state/{runtime_thread_id}")
    ...
    def approve_proposal(self, proposal_id: str, user_id: str) -> None:
        """
        Approve a proposal and apply changes to draft.
        ...
        with psycopg.connect(self.database_url, row_factory=dict_row) as conn:
            with conn.cursor() as cur:
                # Get proposal details and validate access
                cur.execute(
                    """
                    SELECT p.id, p.draft_id, p.status, p.generated_files
                    FROM proposals p
                ...
                # Apply changes to draft (update specification with generated files)
                if proposal["generated_files"]:
                    # In the real implementation, this would update draft_specification_files
                    # For now, we'll just mark the proposal as approved
                    pass
